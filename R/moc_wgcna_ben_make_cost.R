...
# Convert WGCNA scores to edge costs on the ascher graph
# for BEN samples scores
...
uniprot <- gprofiler2::gconvert(unique(moc_wgcna$node_1), target = "UNIPROTSWISSPROT") %>%
  dplyr::arrange(input_number) %>%
  dplyr::distinct(input_number, .keep_all = TRUE)

# drop uniprots that are not in ascher graph
uniprot_as <- uniprot %>%
  dplyr::filter(target %in% V(ascher_graph)$name) %>%
...
moc_wgcna_uni <- moc_wgcna %>%
  dplyr::right_join(uniprot_as, by = c("node_1" = "input")) %>%
  dplyr::select(- node_1) %>%
  dplyr::rename(node_1 = target ) %>%
  dplyr::right_join(uniprot_as, by = c("node_2" = "input")) %>%
  dplyr::select(- node_2) %>%
...
write_tsv(moc_wgcna_uni, "results/wgcna-10th-power-BEN-edgelist-cost-filterDot01.txt")
